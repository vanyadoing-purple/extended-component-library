...
function makePeriod(startDay, startHour, endDay, endHour) {
...
function makeDateInLocale(dateTimeString, utcOffsetMinutes) {
    const sign = utcOffsetMinutes > 0 ? '+' : '-';
    const offsetHelper = new Date();
    offsetHelper.setHours(0);
    offsetHelper.setMinutes(Math.abs(utcOffsetMinutes));
    const offsetString = offsetHelper.toLocaleString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
...
describe('Opening hours utilities', () => {
    describe('formatTimeWithWeekdayMaybe', () => {
        it('formats a relative time with a weekday when the absolute point is over 24 hours in the future', () => {
            ...
            const pointDate = new Date('2023-04-08T17:00-04:00'); // Saturday, 5:00 PM Eastern Time
    ...
    describe('getUpcomingCloseTime', () => {
        it('returns unknown when there isn\'t enough information', async () => {
            ...
            const place = makeFakePlace({ id: '123', regularOpeningHours: undefined, utcOffsetMinutes });
        ...
        it('returns a status when the place is already closed', () => {
            ...
            const place = makeFakePlace({
                id: '123',
                regularOpeningHours: { periods: [mondayNineToFive], weekdayDescriptions: [] },
        ...
        it('returns a closing time for status for a Place closing soon', () => {
            ...
            const regularOpeningHours = {
                periods: [
                    makePeriod(WED, 9, WED, 17),
                    makePeriod(THU, 9, THU, 17),
                    makePeriod(FRI, 9, SAT, 2), // Friday 9am - Sat 2am
                ],
                weekdayDescriptions: []
            };
            const place = makeFakePlace({ id: '123', regularOpeningHours, utcOffsetMinutes: SF_OFFSET });
        ...
        it('returns a closing time for a Place closing in over 24 hours from now', () => {
            ...
            expect(getUpcomingCloseTime(place, monday1pmSf)).toEqual({
                ...
                closeDate: makeDateInLocale('2023-04-14T17:00', SF_OFFSET) // 5pm Fri
            });
        });
    ...
    describe('getUpcomingOpenTime', () => {
        ...
        it('returns a status if the place is already open', () => {
            ...
            const place = makeFakePlace({
                id: '123',
                regularOpeningHours: {
                    periods: [makePeriod(TUE, 9, TUE, 17)],
                    weekdayDescriptions: [],
                },
                utcOffsetMinutes: SF_OFFSET
            });
    ...
    describe('isOpen', () => {
        ...
        it('returns true if the place is open now', () => {
            ...
            const regularOpeningHours = {
                periods: [makePeriod(MON, 9, MON, 17)],
                weekdayDescriptions: []
            };
            const place = makeFakePlace({ id: '123', regularOpeningHours, utcOffsetMinutes: SF_OFFSET });
...
//# sourceMappingURL=opening_hours_test.js.map