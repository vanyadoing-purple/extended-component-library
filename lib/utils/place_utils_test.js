...
describe('isPlaceResult', () => {
    ...
    it('says that a basic PlaceResult is a PlaceResult', () => {
        const placeResult = { place_id: 'some_id', name: 'Name' };
...
describe('priceLevelToNumeric', () => {
    ...
    it('returns null on invalid values', () => {
        expect(priceLevelToNumeric('INVALID')).toEqual(null);
    });
});
...
describe('renderAttribution', () => {
    it('renders <a> tag when URL is defined', () => {
        render(renderAttribution('Provider 1', 'https://www.someprovider.com/1'), document.body);
...
describe('makePlaceFromPlaceResult', () => {
    ...
    it('treats fields missing from PlaceResult as undefined', async () => {
        const place = await makePlaceFromPlaceResult({ geometry: {}, photos: undefined });
    ...
    it('fetches only fields that are not defined by PlaceResult', async () => {
        ...
        const place = await makePlaceFromPlaceResult(SAMPLE_FAKE_PLACE_RESULT);
        await place.fetchFields({ fields: ['displayName', 'location', 'photos', 'servesLunch'] });
        expect(fetchFieldsSpy).toHaveBeenCalledOnceWith({ fields: ['servesLunch'] });
    });
    it('uses the Places Details API when Place doesn\'t support fetchFields', async () => {
        const fetchFieldsSpy = jasmine.createSpy('fetchFields')
            .and.rejectWith(new Error('Place.prototype.fetchFields() is not available, try the beta channel.'));
        const fakeGetDetails = (options, callback) => {
            callback({ price_level: 2 }, 'OK');
        };
        const placeDetailsSpy = jasmine.createSpy('placeDetails').and.callFake(fakeGetDetails);
        const fakePlacesLibrary = {
            Place: class {
                constructor(options) {
                    return makeFakePlace({ id: options.id, fetchFields: fetchFieldsSpy });
            ...
            PlacesService: class {
                constructor() {
                    this.getDetails = placeDetailsSpy;
                }
            }
        };
        env.importLibrarySpy?.and.resolveTo(fakePlacesLibrary);
        const place = await makePlaceFromPlaceResult({ place_id: '123', url: 'http://foo/bar', rating: 3.5 });
        // NOTE: `attributions` -> `html_attributions` should *not* be requested,
        // as the Places Details API considers it an invalid field mask.
        await place.fetchFields({ fields: ['rating', 'priceLevel', 'attributions'] });
        expect(placeDetailsSpy)
            .toHaveBeenCalledOnceWith({ placeId: '123', fields: ['price_level'] }, jasmine.any(Function));
    ...
    it('calls the utility isOpen function when isOpen() isn\'t implemented on Place', async () => {
        const isOpenSpy = jasmine.createSpy('isOpen').and.rejectWith(new Error('Place.prototype.isOpen() is not available, try the beta channel.'));
        const fakePlacesLibrary = {
            Place: class {
                constructor(options) {
                    return makeFakePlace({ id: options.id, isOpen: isOpenSpy });
...
//# sourceMappingURL=place_utils_test.js.map