...
describe('RouteOverview', () => {
    ...
    async function prepareState(template) {
        const errorSpy = spyOn(LoggingController.prototype, 'error');
        const root = env.render(template ?? html `<gmpx-route-overview></gmpx-route-overview>`);
        await env.waitForStability();
        const overview = root.querySelector('gmpx-route-overview');
        const provider = overview.shadowRoot.querySelector('gmpx-route-data-provider');
    ...
    it('logs an error when setting multiple origins', async () => {
      ...
      <gmpx-route-overview
          ...
          origin-address="456"
      ></gmpx-route-overview>`);
        expect(errorSpy).toHaveBeenCalledOnceWith('Too many origins. Only one of origin-lat-lng, ' +
    ...
    it('logs an error when setting multiple destinations', async () => {
      ...
      <gmpx-route-overview
          ...
          origin-lat-lng="3,4"
      ></gmpx-route-overview> `);
        expect(errorSpy).toHaveBeenCalledOnceWith('Too many destinations. Only one of destination-lat-lng, ' +
    ...
    it('sets marker titles to the origin/destination queries', async () => {
      ...
      <gmpx-route-overview destination-address="abc" origin-address="123">
        ...
        const originMarkers = Array.from(overview.shadowRoot.querySelectorAll('gmpx-route-marker[waypoint="origin"]'));
        const destinationMarkers = Array.from(overview.shadowRoot.querySelectorAll('gmpx-route-marker[waypoint="destination"]'));
    ...
    it('creates successive overviews with increasing z-index', async () => {
        ...
        const secondOverview = root.appendChild(document.createElement('gmpx-route-overview'));
        const overviewMaxIndex = Math.max(...consumerZIndices(overview));
        const secondOverviewMinIndex = Math.min(...consumerZIndices(secondOverview));
    ...
    it('renders the right number of markers and polylines', async () => {
        ...
        const originMarkers = overview.shadowRoot.querySelectorAll('gmpx-route-marker[waypoint="origin"]');
        const destinationMarkers = overview.shadowRoot.querySelectorAll('gmpx-route-marker[waypoint="destination"]');
        const polylines = overview.shadowRoot.querySelectorAll('gmpx-route-polyline');
 ...
 */
function consumerZIndices(overview) {
    const consumers = Array.from(overview.shadowRoot.querySelectorAll('gmpx-route-marker,gmpx-route-polyline'));
    return consumers.map((el) => el.zIndex)
        .filter((i) => (typeof i === 'number'));
}
//# sourceMappingURL=route_overview_test.js.map