...
describe('PlacePicker', () => {
    ...
    async function prepareState(template) {
        const root = env.render(template ?? html `<gmpx-place-picker></gmpx-place-picker>`);
        const picker = root.querySelector('gmpx-place-picker');
        await env.waitForStability();
        const input = picker.renderRoot.querySelector('input');
        const searchButton = picker.renderRoot.querySelector('.search-button');
        const clearButton = picker.renderRoot.querySelector('.clear-button');
        return { root, picker, input, searchButton, clearButton };
    }
    async function enterQueryText(input, query = 'some text') {
        input.value = query;
    ...
    it(`resets the place type to none (= all) given a falsy value`, async () => {
        const { picker } = await prepareState(html `<gmpx-place-picker type="restaurant"></gmpx-place-picker>`);
        picker.type = '';
        await env.waitForStability();
        expect(env.fakeGoogleMapsHarness.autocompleteSpy.setOptions)
    ...
    it(`sets value based on user selection and fires event`, async () => {
        const dispatchEventSpy = spyOn(PlacePicker.prototype, 'dispatchEvent');
        let autocompleteSelectionHandler;
        ...
        env.fakeGoogleMapsHarness.autocompleteSpy.getPlace.and.returnValue(FAKE_PLACE_RESULT_FROM_AUTOCOMPLETE);
        const { picker, input, searchButton, clearButton } = await prepareState();
        await enterQueryText(input);
    ...
    it(`sets value based on place returned by Find Place request`, async () => {
        env.fakeGoogleMapsHarness.autocompleteSpy.getBounds.and.returnValue(FAKE_BOUNDS);
        const { picker, input, searchButton, clearButton } = await prepareState();
        await enterQueryText(input, '123 Main St');
        const fetchFieldsSpy = spyOn(FAKE_PLACE_FROM_QUERY, 'fetchFields').and.callThrough();
        searchButton.click();
        await env.waitForStability();
    ...
    it('sets value from fallback GA API when Place.searchByText is not available', async () => {
        env.fakeGoogleMapsHarness.autocompleteSpy.getBounds.and.returnValue(FAKE_BOUNDS);
        const { picker, input, searchButton, clearButton } = await prepareState();
        env.fakeGoogleMapsHarness.searchByTextHandler
            .and.throwError(new Error('google.maps.places.Place.searchByText() is not available in the SDK!'));
    ...
    it(`dispatches request error event when search is rejected`, async () => {
        ...
        expect(clearButton.hidden).toBeFalse();
        expect(dispatchEventSpy)
            .toHaveBeenCalledOnceWith(jasmine.objectContaining({ type: 'gmpx-requesterror', error }));
    });
    ...
    `);
        const mapElement = root.querySelector('gmp-map');
        expect(env.fakeGoogleMapsHarness.autocompleteSpy.bindTo)
...
//# sourceMappingURL=place_picker_test.js.map