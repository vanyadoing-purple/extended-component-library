...
let TestAPIConsumer = class TestAPIConsumer extends BaseComponent {
    async connectedCallback() {
        super.connectedCallback();
        this.coreLibraryPromise = APILoader.importLibrary('core', this);
        this.coreLibrary = await this.coreLibraryPromise;
    }
};
TestAPIConsumer = __decorate([
...
describe('APILoader', () => {
    ...
    async function prepareState(config) {
      ...
      ${when(config?.renderConsumerEl ?? true, () => html `<gmpx-test-api-consumer></gmpx-test-api-consumer>`)}
    `);
        const loaderEl = root.querySelector('gmpx-api-loader');
        const consumerEl = root.querySelector('gmpx-test-api-consumer');
    ...
    it('loads API with minimum configuration and empty key', async () => {
        const { scriptLoadSpy, consoleWarnSpy, importLibrarySpy } = await prepareState({ loaderHTML: html `<gmpx-api-loader key=""></gmpx-api-loader>` });
        expect(scriptLoadSpy).toHaveBeenCalledOnceWith({
            key: '',
            v: 'beta',
            solutionChannel: `GMP_${ATTRIBUTION_SOURCE_ID}_extended_v${LIBRARY_VERSION}`,
        });
    ...
    it('loads API when properties are set programmatically', async () => {
        const { loaderEl, consumerEl, scriptLoadSpy, consoleWarnSpy } = await prepareState();
    ...
    it('logs warning when `google.maps` is already defined', async () => {
        ...
        expect(consoleWarnSpy)
            .toHaveBeenCalledOnceWith('<gmpx-api-loader>: ' +
    ...
    it('logs warning when `google.maps` is available after delay', async () => {
        const fakeSdk = env.fakeGoogleMapsHarness.sdk;
        const { consoleWarnSpy, importLibrarySpy } = await prepareState({ loaderHTML: html `` });
        ...
        expect(importLibrarySpy).toHaveBeenCalledWith('marker');
        expect(consoleWarnSpy)
            .toHaveBeenCalledOnceWith('<gmpx-test-api-consumer>: ' +
    ...
    it('logs warning when a property is changed after API load', async () => {
        ...
        expect(consoleWarnSpy)
            .toHaveBeenCalledOnceWith(`<gmpx-api-loader>: Property 'region' cannot be updated once ` +
    ...
    it('logs warning when multiple API loaders are present', async () => {
        ...
        expect(consoleWarnSpy)
            ...
            'please ensure you only have a single instance.', loaderEl);
    });
    it('rejects promise when API is requested without a loader', async () => {
        const { consumerEl, scriptLoadSpy, consoleWarnSpy } = await prepareState({ loaderHTML: html `` });
        await expectAsync((async () => {
            waitForAnyPolling();
            return await consumerEl.coreLibraryPromise;
        })())
            .toBeRejectedWithError('<gmpx-test-api-consumer>: ' +
    ...
    it('resolves with library when API is requested programmatically with loader', async () => {
        const { loaderEl, consoleWarnSpy } = await prepareState({ renderConsumerEl: false });
        loaderEl.key = 'TEST_API_KEY';
        waitForAnyPolling();
    ...
    it('rejects promise when API is requested programmatically without a loader', async () => {
        const { scriptLoadSpy, consoleWarnSpy } = await prepareState({ loaderHTML: html ``, renderConsumerEl: false });
        await expectAsync((async () => {
            const libraryPromise = APILoader.importLibrary('core');
            waitForAnyPolling();
            return await libraryPromise;
        })())
            .toBeRejectedWithError('APILoader.importLibrary(): Unable to initialize the Google ' +
...
//# sourceMappingURL=api_loader_test.js.map