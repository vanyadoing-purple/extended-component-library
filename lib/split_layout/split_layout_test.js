...
describe('SplitLayout', () => {
    ...
    `);
        ...
        const splitLayout = root.querySelector('gmpx-split-layout');
        return {
            splitLayout,
            layoutDiv: splitLayout.renderRoot.querySelector('.layout'),
            mainContainer: splitLayout.renderRoot.querySelector('.main-container'),
            fixedContainer: splitLayout.renderRoot.querySelector('.fixed-container'),
        };
    }
    it('is defined', () => {
    ...
    it('renders row layout in correct DOM order and direction', async () => {
        const { layoutDiv, mainContainer, fixedContainer } = await prepareState({ componentWidth: '800px' });
        expect(getComputedStyle(layoutDiv).flexDirection).toBe('row');
        expect(mainContainer.compareDocumentPosition(fixedContainer) &
            Node.DOCUMENT_POSITION_PRECEDING)
            .toBeTruthy();
    });
    it('renders column layout in correct DOM order and direction', async () => {
        const { layoutDiv, mainContainer, fixedContainer } = await prepareState({ componentWidth: '400px' });
    ...
    it('flips row layout when `row-reverse` attribute is set', async () => {
        const { layoutDiv, mainContainer, fixedContainer } = await prepareState({ componentWidth: '800px', rowReverse: true });
        expect(getComputedStyle(layoutDiv).flexDirection).toBe('row');
        expect(mainContainer.compareDocumentPosition(fixedContainer) &
            Node.DOCUMENT_POSITION_FOLLOWING)
            .toBeTruthy();
    ...
    it('flips column layout when `column-reverse` attribute is set', async () => {
        const { layoutDiv, mainContainer, fixedContainer } = await prepareState({ componentWidth: '400px', columnReverse: true });
        expect(getComputedStyle(layoutDiv).flexDirection).toBe('column');
        expect(mainContainer.compareDocumentPosition(fixedContainer) &
            Node.DOCUMENT_POSITION_PRECEDING)
            .toBeTruthy();
    ...
    it('updates layout when breakpoint changes', async () => {
        const { splitLayout, layoutDiv } = await prepareState({ componentWidth: '400px' });
        splitLayout.rowLayoutMinWidth = 400;
        await env.waitForStability();
        expect(getComputedStyle(layoutDiv).flexDirection).toBe('row');
    });
});
//# sourceMappingURL=split_layout_test.js.map