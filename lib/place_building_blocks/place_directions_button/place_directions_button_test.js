...
describe('PlaceDirectionsButton', () => {
    ...
    `);
        ...
        return root
            .querySelector('gmpx-place-directions-button')
            .renderRoot.querySelector('gmpx-icon-button');
    }
    it('is defined', () => {
    ...
    it(`opens directions to place's address if defined`, async () => {
        ...
        expect(iconButton.href)
            .toBe('https://www.google.com/maps/dir/?api=1&destination_place_id=FAKE_PLACE_ID&destination=123%20Main%20Street');
    });
    it(`opens directions to place's display name without address`, async () => {
        ...
        const iconButton = await prepareState({ place });
        expect(iconButton.href)
            .toBe('https://www.google.com/maps/dir/?api=1&destination_place_id=FAKE_PLACE_ID&destination=Fake%20Place');
    });
    it(`opens directions to place's location without address/name`, async () => {
        ...
        const iconButton = await prepareState({ place });
        expect(iconButton.href)
            .toBe('https://www.google.com/maps/dir/?api=1&destination_place_id=FAKE_PLACE_ID&destination=-12.34,56.78');
    });
    it(`always includes the destination URL param when a Place ID is specified`, async () => {
        // tslint:disable-next-line:prefer-type-annotation
        const place = { id: 'FAKE_PLACE_ID' };
        const iconButton = await prepareState({ place });
        expect(iconButton.href)
            .toBe('https://www.google.com/maps/dir/?api=1&destination_place_id=FAKE_PLACE_ID&destination=Selected%20Place');
    });
    const originParams = [
        {
            dataType: 'LatLng',
    ...
    const destinationParams = [
        ...
        },
        {
            ...
            expectedQuery: '&destination_place_id=FAKE_DESTINATION_ID&destination=-12.34,56.78',
        }
    ];
    destinationParams.forEach(({ dataType, destination, expectedQuery }) => {
        it(`opens reversed directions to a ${dataType} destination`, async () => {
            ...
            const iconButton = await prepareState({ place, origin: destination, reversed: true });
            expect(iconButton.href)
                .toBe(`https://www.google.com/maps/dir/?api=1&origin_place_id=FAKE_PLACE_ID&origin=Fake%20Place${expectedQuery}`);
        });
    });
    ...
    `);
        await env.waitForStability();
        const el = root.querySelector('gmpx-place-directions-button');
        expect(el.role).toBe('none');
    });
});
//# sourceMappingURL=place_directions_button_test.js.map