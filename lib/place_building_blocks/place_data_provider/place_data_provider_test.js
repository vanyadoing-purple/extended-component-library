...
let TestConsumer = class TestConsumer extends PlaceDataConsumer {
    constructor() {
        super(...arguments);
        this.updateCount = 0;
    }
...
], TestConsumer.prototype, "field", void 0);
TestConsumer = __decorate([
    customElement('gmpx-test-consumer')
], TestConsumer);
describe('PlaceDataProvider', () => {
    const env = new Environment();
    function attachFetchFieldsSpy(place, fetchFieldsSpy) {
        fetchFieldsSpy.and.callFake(async ({ fields }) => {
    ...
    async function prepareState(template, fetchFields) {
    ...
    `);
        const consumer = provider.querySelector('gmpx-test-consumer');
    ...
    it('fetches from Place Details when Place.fetchFields() is not available', async () => {
        ...
        const { provider } = await prepareState(html `
            ...
            throw new Error('Place.prototype.fetchFields() is not available in the SDK!');
        });
...
//# sourceMappingURL=place_data_provider_test.js.map