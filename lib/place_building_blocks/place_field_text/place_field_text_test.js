...
import { PLACE_RESULT_TEXT_FIELDS, PLACE_TEXT_FIELDS, PlaceFieldText } from './place_field_text.js';
function getText(root, field) {
    if (field === undefined) {
        return root.querySelector('gmpx-place-field-text').renderRoot.textContent;
    }
    else {
        const query = `gmpx-place-field-text[field="${field}"]`;
        return root.querySelector(query).renderRoot.textContent;
    }
}
...
describe('PlaceFieldText', () => {
    ...
    it('renders all fields using a fake Place and PlaceResult fields', async () => {
        const root = env.render(html `
    ...
    it('renders all fields using a PlaceResult and PlaceResult fields', async () => {
        const root = env.render(html `
         ${map(PLACE_RESULT_TEXT_FIELDS, field => html `
           <gmpx-place-field-text .place=${placeResult} field="${field}">
           </gmpx-place-field-text>
    ...
    it('renders nothing when the field name is invalid', async () => {
        const place = makeFakePlace({ id: '1234567890' });
        const root = env.render(html `
      <gmpx-place-field-text field="${'badField'}" .place=${place}>
      </gmpx-place-field-text>
    `);
        await env.waitForStability();
        expect(getText(root)).toBe('');
    ...
    it('renders all business statuses correctly', async () => {
        ...
        const [operationalField, permanentField, temporaryField] = await prepareState(html `
...
//# sourceMappingURL=place_field_text_test.js.map