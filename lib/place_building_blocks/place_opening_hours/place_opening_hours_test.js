...
describe('PlaceOpeningHours', () => {
    ...
    async function prepareState(configs) {
    ...
    `);
        ...
        return root.querySelector('gmpx-place-opening-hours');
    }
    function getClosedSlotText(el) {
        return el.renderRoot.querySelector('div[slot="false"]')?.textContent;
    }
    function getOpenSlotText(el) {
    ...
    it('renders nothing when place is operational but opening hours is missing', async () => {
        const place = makeFakePlace({ ...FAKE_PLACE_PROPS, regularOpeningHours: undefined });
    ...
    it('renders business status when place is temporarily closed', async () => {
        ...
        const el = await prepareState({ place });
        const fieldText = el.renderRoot.querySelector('gmpx-place-field-text');
    ...
    it('renders default message when missing UTC offset', async () => {
        const place = makeFakePlace({ ...FAKE_PLACE_PROPS, utcOffsetMinutes: undefined });
        const el = await prepareState({ place });
    ...
    it('renders based on `isOpen()` when opening hours is present', async () => {
        ...
        const el = await prepareState({ place });
        const fieldBoolean = el.renderRoot.querySelector('gmpx-place-field-boolean');
    ...
    it('renders no button or details div with `summaryOnly` set', async () => {
        const el = await prepareState({ place: makeFakePlace(FAKE_PLACE_PROPS), summaryOnly: true });
    ...
    it('renders button and details div without `summaryOnly` set', async () => {
        const el = await prepareState({ place: makeFakePlace(FAKE_PLACE_PROPS), summaryOnly: false });
        const button = el.renderRoot.querySelector('button');
    ...
    it('renders weekly opening hours inside details div', async () => {
        ...
        expect(details?.textContent)
            .toHaveNormalizedText(FAKE_PLACE_PROPS.regularOpeningHours.weekdayDescriptions.join(' '));
    });
...
//# sourceMappingURL=place_opening_hours_test.js.map