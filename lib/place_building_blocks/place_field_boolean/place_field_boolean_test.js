...
import { PLACE_BOOLEAN_FIELDS, PlaceFieldBoolean } from './place_field_boolean.js';
function expectTrueSlot(el) {
    const slots = el.renderRoot.querySelectorAll('slot');
    expect(slots.length).toBe(1);
    expect(slots[0].name).toBe('true');
}
function expectFalseSlot(el) {
    const slots = el.renderRoot.querySelectorAll('slot');
    expect(slots.length).toBe(1);
    expect(slots[0].name).toBe('false');
}
function expectNoSlots(el) {
    const slots = el.renderRoot.querySelectorAll('slot');
...
describe('place field boolean test', () => {
    ...
    it('renders no slot when the field name is invalid', async () => {
        const place = makeFakePlace({ id: '1234567890' });
        const [el] = await prepareState(html `
      <gmpx-place-field-boolean field="${'badField'}"
                                .place=${place}>
      </gmpx-place-field-boolean>
    `);
        expectNoSlots(el);
    ...
    it(`doesn't call isOpen if it would call the API`, async () => {
      ...
      <gmpx-place-field-boolean field="isOpen()" .place=${noBusinessStatusPlace}>
      </gmpx-place-field-boolean>
    ...
    it('polls when displaying isOpen()', async () => {
        ...
        const [booleanEl] = await prepareState(html `<gmpx-place-field-boolean field="isOpen()" .place=${place}></gmpx-place-field-boolean>`);
    ...
    it('performs only one render for a synchronously accessed property', async () => {
    ...
    it('performs two renders for an asynchronously accessed property', async () => {
        const place = makeFakePlace({
            id: '1234567890',
            businessStatus: 'OPERATIONAL',
            regularOpeningHours: { periods: [], weekdayDescriptions: [] },
...
//# sourceMappingURL=place_field_boolean_test.js.map