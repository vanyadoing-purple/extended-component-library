...
describe('PlacePhotoGallery', () => {
    ...
    `);
        ...
        const gallery = root.querySelector('gmpx-place-photo-gallery');
        const tiles = gallery.renderRoot.querySelectorAll('[part="tile"]');
        const lightbox = gallery.renderRoot.querySelector('.lightbox');
    ...
    it('shows all photos from PlaceResult data as tiles, in order', async () => {
        const getUrl0Spy = spyOn(fakePlaceResult.photos[0], 'getUrl').and.callThrough();
        const getUrl1Spy = spyOn(fakePlaceResult.photos[1], 'getUrl').and.callThrough();
        const getUrl2Spy = spyOn(fakePlaceResult.photos[2], 'getUrl').and.callThrough();
    ...
    it('opens lightbox with corresponding Photo on tile click', async () => {
        ...
        expect(lightbox?.querySelector('.title')?.innerText)
            .toBe('Place Name');
        expect(lightbox?.querySelector('.caption')?.innerText)
            .toBe('Photo by Author B1');
    });
    it('opens lightbox with corresponding PlacePhoto on tile click', async () => {
        const { lightbox } = await prepareState({ place: fakePlaceResult, clickOnTile: 1 });
        expect(lightbox?.open).toBe(true);
        expect(lightbox?.querySelector('img')?.src)
            .toBe('https://lh3.googleusercontent.com/places/B');
        expect(lightbox?.querySelector('.title')?.innerText)
    ...
    it('navigates to previous via right arrow key in RTL mode', async () => {
        const { lightbox } = await prepareState({ rtl: true, place: fakePlace, clickOnTile: 1 });
        document.dispatchEvent(new KeyboardEvent('keydown', { 'key': 'ArrowRight' }));
        await env.waitForStability();
        expect(lightbox?.querySelector('img')?.src)
            .toBe('https://lh3.googleusercontent.com/places/A');
    ...
    it('navigates to previous photo via previous button', async () => {
        const { lightbox } = await prepareState({ place: fakePlace, clickOnTile: 1 });
        lightbox?.querySelector('[aria-label="Previous"]')
            ?.click();
        await env.waitForStability();
        expect(lightbox?.querySelector('img')?.src)
            .toBe('https://lh3.googleusercontent.com/places/A');
    ...
    it('navigates to next photo via next button', async () => {
        const { lightbox } = await prepareState({ place: fakePlace, clickOnTile: 1 });
        lightbox?.querySelector('[aria-label="Next"]')?.click();
        await env.waitForStability();
        expect(lightbox?.querySelector('img')?.src)
            .toBe('https://lh3.googleusercontent.com/places/C');
    });
    it('disables previous button on first photo', async () => {
        ...
        expect(lightbox?.querySelector('[aria-label="Previous"]')
            ?.disabled)
            .toBeTrue();
        expect(lightbox?.querySelector('[aria-label="Next"]')
            ?.disabled)
            .toBeFalse();
    });
    ...
    it('renders PlacePhoto attribution as link iff there is URI', async () => {
        const { lightbox } = await prepareState({ place: fakePlaceResult, clickOnTile: 0 });
        const caption = lightbox?.querySelector('.caption');
        expect(caption.textContent)
            .toHaveNormalizedText('Photo by Author A1 Author A2');
        const anchors = caption.querySelectorAll('a');
...
//# sourceMappingURL=place_photo_gallery_test.js.map