...
function simpleStripComments(xml) {
 ...
 */
function renderedHTML(renderRoot) {
...
describe('place attribution test', () => {
    ...
    it('renders nothing when there is no attribution data', async () => {
        ...
        const el = await prepareState(html `<gmpx-place-attribution .place=${place}></gmpx-place-attribution>`);
    ...
    it('renders a link and non-link attribution', async () => {
        ...
        expect(simpleStripComments(renderedHTML(el.renderRoot)))
            .toEqual('<a target="_blank" href="https://foo.com">Foo</a>' +
            '<span class="sep">, </span><span>Bar</span>');
    });
});
//# sourceMappingURL=place_attribution_test.js.map