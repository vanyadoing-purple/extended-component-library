...
describe('RoutePolyline', () => {
    ...
    async function prepareState(template) {
        const setMapSpy = spyOn(env.fakeGoogleMapsHarness.libraries['maps'].Polyline.prototype, 'setMap');
        const setOptionsSpy = spyOn(env.fakeGoogleMapsHarness.libraries['maps'].Polyline.prototype, 'setOptions');
        const setPathSpy = spyOn(env.fakeGoogleMapsHarness.libraries['maps'].Polyline.prototype, 'setPath');
        const constructorSpy = spyOn(env.fakeGoogleMapsHarness.libraries['maps'], 'Polyline')
            .and.callThrough();
        const root = env.render(template ?? html `<gmpx-route-polyline></gmpx-route-polyline>`);
        const map = root.querySelector('gmp-map');
        const polyline = root.querySelector('gmpx-route-polyline');
    ...
    it('concatenates the paths from multiple steps', async () => {
        ...
        polyline.route = makeFakeRoute({
            legs: [makeFakeLeg({
                    steps: [makeFakeStep({ path: [ll1, ll2] }), makeFakeStep({ path: [ll3, ll4] })]
                })]
        });
        await env.waitForStability();
    ...
    describe('viewport management', () => {
      ...
      `);
            expect(fitBoundsSpy.calls.mostRecent()?.args[0])
                .toEqual(new FakeLatLngBounds({ north: 2, south: 0, east: 2, west: 0 }));
        });
      ...
      `);
            ...
            await env.waitForStability();
            expect(fitBoundsSpy.calls.mostRecent()?.args[0])
                .toEqual(new FakeLatLngBounds({ north: 2, south: 1, east: 2, west: 1 }));
        });
      ...
      `);
            ...
            await env.waitForStability();
            expect(fitBoundsSpy.calls.mostRecent()?.args[0])
                .toEqual(new FakeLatLngBounds({ north: 3, south: 2, east: 3, west: 2 }));
        });
    });
});
//# sourceMappingURL=route_polyline_test.js.map