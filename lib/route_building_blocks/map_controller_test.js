...
let TestMapControllerHost = class TestMapControllerHost extends LitElement {
    constructor() {
        super(...arguments);
        this.mapController = new MapController(this);
    }
};
TestMapControllerHost = __decorate([
    customElement('gmpx-test-map-controller-host')
], TestMapControllerHost);
describe('MapController', () => {
    const env = new Environment();
    beforeAll(() => {
        env.defineFakeMapElement();
    });
    async function prepareMapElement() {
    ...
    it('waits for <gmp-map> to be defined', async () => {
        const whenGmpMapDefined = new Deferred();
    ...
    `);
        ...
        const host = root.querySelector('gmpx-test-map-controller-host');
        expect(host.mapController.map).toBe(mapElement.innerMap);
    });
...
//# sourceMappingURL=map_controller_test.js.map