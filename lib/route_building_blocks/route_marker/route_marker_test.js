...
function fakeRouteBetween([startLat, startLng], [endLat, endLng]) {
...
describe('RouteMarker', () => {
    ...
    async function prepareState(template) {
        const constructorSpy = spyOn(env.fakeGoogleMapsHarness.libraries['marker'], 'AdvancedMarkerElement')
            .and.callThrough();
        const root = env.render(template ?? html `<gmpx-route-marker></gmpx-route-marker>`);
        await env.waitForStability();
        const marker = root.querySelector('gmpx-route-marker');
    ...
    it('sets position to origin via attribute', async () => {
      ...
      <gmpx-route-marker waypoint="origin"
        ...
        // clang-format on
        expect(innerMarker.position.lat()).toEqual(1);
        expect(innerMarker.position.lng()).toEqual(2);
    });
    it('sets position to destination via attribute', async () => {
      ...
      <gmpx-route-marker waypoint="destination"
        ...
        // clang-format on
        expect(innerMarker.position.lat()).toEqual(3);
        expect(innerMarker.position.lng()).toEqual(4);
    });
    ...
    it('updates position on route change', async () => {
      ...
      <gmpx-route-marker waypoint="origin"
        ...
        await env.waitForStability();
        expect(innerMarker.position.lat()).toEqual(5);
        expect(innerMarker.position.lng()).toEqual(6);
    });
    ...
    it('logs an error on invalid waypoint value', async () => {
      ...
      <gmpx-route-marker .waypoint=${'invalid'}
        .route=${fakeRouteBetween([1, 2], [3, 4])}>
      </gmpx-route-marker>`);
        // clang-format on
        expect(LoggingController.prototype.error)
            .toHaveBeenCalledOnceWith('Unsupported waypoint "invalid". ' +
...
//# sourceMappingURL=route_marker_test.js.map