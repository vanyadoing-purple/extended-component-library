...
let FakeRouteDataConsumer = class FakeRouteDataConsumer extends LitElement {
};
__decorate([
    consume({ context: routeContext, subscribe: true }),
    property({ attribute: false }),
    __metadata("design:type", Object)
], FakeRouteDataConsumer.prototype, "contextRoute", void 0);
FakeRouteDataConsumer = __decorate([
    customElement('gmpx-fake-route-data-consumer')
], FakeRouteDataConsumer);
describe('RouteDataProvider', () => {
    ...
    async function prepareState(template) {
        ...
        const provider = root.querySelector('gmpx-route-data-provider');
    ...
    it('fetches a route with lat-lng attributes', async () => {
      ...
      <gmpx-route-data-provider origin-lat-lng="1,2" destination-lat-lng="3,4">
        ...
        expect(arg.origin.location)
            .toEqual({ lat: 1, lng: 2 });
        expect(arg.destination.location)
            .toEqual({ lat: 3, lng: 4 });
    ...
    it('fetches a route with place-id attributes', async () => {
      ...
      <gmpx-route-data-provider origin-place-id="abc" destination-place-id="123">
        ...
        expect(routeSpy).toHaveBeenCalledTimes(1);
        const arg = routeSpy.calls.mostRecent().args[0];
        expect(arg.origin.placeId).toEqual('abc');
        expect(arg.destination.placeId).toEqual('123');
        expect(provider.contextRoute).toBe(FAKE_ROUTE);
    });
    it('fetches a route with placeId properties', async () => {
        ...
        expect(routeSpy).toHaveBeenCalledTimes(1);
        const arg = routeSpy.calls.mostRecent().args[0];
        expect(arg.origin.placeId).toEqual('def');
        expect(arg.destination.placeId).toEqual('456');
        expect(provider.contextRoute).toBe(FAKE_ROUTE);
    });
    it('fetches a route with address attributes', async () => {
      ...
      <gmpx-route-data-provider origin-address="abc" destination-address="123">
        ...
        expect(routeSpy).toHaveBeenCalledTimes(1);
        const arg = routeSpy.calls.mostRecent().args[0];
        expect(arg.origin.query).toEqual('abc');
        expect(arg.destination.query).toEqual('123');
        expect(provider.contextRoute).toBe(FAKE_ROUTE);
    });
    it('fetches a route with address properties', async () => {
        ...
        expect(routeSpy).toHaveBeenCalledTimes(1);
        const arg = routeSpy.calls.mostRecent().args[0];
        expect(arg.origin.query).toEqual('def');
        expect(arg.destination.query).toEqual('456');
        expect(provider.contextRoute).toBe(FAKE_ROUTE);
    });
    ...
    it('provides a route via the route property', async () => {
      ...
      <gmpx-route-data-provider>
        ...
        const consumer = root.querySelector('gmpx-fake-route-data-consumer');
...
//# sourceMappingURL=route_data_provider_test.js.map