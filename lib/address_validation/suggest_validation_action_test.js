...
import { ConfirmationLevel, Granularity } from '../utils/googlemaps_types.js';
...
function makeFakeValidationResponse(address, verdict) {
    return {
        result: {
            verdict,
            address: {
...
describe('SuggestValidationAction', () => {
    it('returns FIX when an address is missing a non-subpremise component', () => {
        const suggestion = suggestValidationAction(makeFakeValidationResponse({ missingComponentTypes: ['locality'] }, GOOD_VERDICT));
        expect(suggestion.suggestedAction).toBe(SuggestedAction.FIX);
    });
    it('returns FIX when the validation granularity is OTHER', () => {
        const suggestion = suggestValidationAction(makeFakeValidationResponse({}, { ...GOOD_VERDICT, validationGranularity: Granularity.OTHER }));
        expect(suggestion.suggestedAction).toBe(SuggestedAction.FIX);
    });
    ...
    it('returns FIX when there is an unresolved token', () => {
        const suggestion = suggestValidationAction(makeFakeValidationResponse({ unresolvedTokens: ['foobar'] }, GOOD_VERDICT));
        expect(suggestion.suggestedAction).toBe(SuggestedAction.FIX);
    });
    ...
    it('returns CONFIRM when there is a replaced component', () => {
        const suggestion = suggestValidationAction(makeFakeValidationResponse({}, { ...GOOD_VERDICT, hasReplacedComponents: true }));
        expect(suggestion.suggestedAction).toBe(SuggestedAction.CONFIRM);
    });
    ...
    it('returns ACCEPT for a complete verdict without any issues', () => {
        const suggestion = suggestValidationAction(makeFakeValidationResponse({}, GOOD_VERDICT));
        expect(suggestion.suggestedAction).toBe(SuggestedAction.ACCEPT);
    });
});
//# sourceMappingURL=suggest_validation_action_test.js.map