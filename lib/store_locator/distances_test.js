...
describe('DistanceMeasurer', () => {
    ...
    it('locally caches Distance Matrix requests', async () => {
        ...
        const distances = await measurer.computeDistances(origin, destinations, units);
    ...
    it('does not cache a transient error, OVER_QUERY_LIMIT', async () => {
        const distanceMatrixSpy = spyOn(env.fakeGoogleMapsHarness, 'distanceMatrixHandler')
            .and.throwError({ code: 'OVER_QUERY_LIMIT', name: 'MapsRequestError' });
        const origin = { lat: 0, lng: 0 };
        const destinations = [{ lat: 1, lng: 1 }, { lat: 2, lng: 2 }];
        const units = 0;
        const measurer = new DistanceMeasurer();
    ...
    it('caches a hard error, INVALID_REQUEST', async () => {
        const distanceMatrixSpy = spyOn(env.fakeGoogleMapsHarness, 'distanceMatrixHandler')
            .and.throwError({ code: 'INVALID_REQUEST', name: 'MapsRequestError' });
        const origin = { lat: 0, lng: 0 };
        const destinations = [{ lat: 1, lng: 1 }, { lat: 2, lng: 2 }];
        const units = 0;
        const measurer = new DistanceMeasurer();
    ...
    it('only uses Distance Matrix for the closest 25 destinations', async () => {
        ...
        expect(distances[5])
            .toEqual({ source: DistanceSource.DISTANCE_MATRIX, value: 4, text: '4 0' });
    });
});
//# sourceMappingURL=distances_test.js.map