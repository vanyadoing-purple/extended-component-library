...
let TestPlaceOverviewChild = class TestPlaceOverviewChild extends PlaceDataConsumer {
    getPlaceFromContext() {
...
TestPlaceOverviewChild = __decorate([
    customElement('gmpx-test-place-overview-child')
], TestPlaceOverviewChild);
describe('PlaceOverview', () => {
    ...
    `);
        ...
        return root.querySelector('gmpx-place-overview');
    }
    it('is defined', () => {
    ...
    it('logs warning when a child is in the default slot', async () => {
        ...
        const overview = await prepareState({ slottedContent: html `<div id="invalid"></div>` });
        await env.waitForStability();
        const invalidChild = overview.querySelector('#invalid');
        expect(consoleWarnSpy)
            .toHaveBeenCalledWith('<gmpx-place-overview>: ' +
            'Detected child element in unsupported default slot. ' +
            'This component supports the following slots: "action".', invalidChild);
    });
    it('logs error and resets when size is set to an invalid value', async () => {
        const consoleErrorSpy = spyOn(console, 'error');
        const overview = await prepareState({ size: 'foo' });
        await env.waitForStability();
    ...
    `);
        await env.waitForStability();
        const overview = root.querySelector('gmpx-place-overview');
        const innerProvider = overview.renderRoot.querySelector('gmpx-place-data-provider');
    ...
    `);
        await env.waitForStability();
        const child = root.querySelector('gmpx-test-place-overview-child');
    ...
    `);
        await env.waitForStability();
        const overviews = root.querySelectorAll('gmpx-place-overview');
...
//# sourceMappingURL=place_overview_test.js.map